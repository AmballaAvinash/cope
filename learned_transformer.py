...
def train_learned_classifier_glue(train, val, num_epochs, save_path, num_positions = 512, num_classes =2 ,  loss = "CE",use_token_type_embeddings =True):
    """
...
def decode_learned_glue(state_dict, dataloader,num_positions = 512, num_classes =2 , loss = "CE", use_token_type_embeddings =True):
    model_config = {
        'vocab_size': tokenizer.vocab_size,  # BERT vocab size, adjust based on your tokenizer
        'num_positions': num_positions,
        'd_model': 256,
...
def attention_plots_learned(state_dict, valdata,save_path, num_positions = 512, num_classes =2 , use_token_type_embeddings =True):
    ...
    with torch.no_grad():
            ...
            for j in range(len(attention_maps)):
                for i, attn_map in enumerate(attention_maps[j]):
                    ...
                    plt.tight_layout()
                    plt.savefig(f"{save_path}/learned_sample{i}_attn_{j}.png")
                    plt.close(fig)