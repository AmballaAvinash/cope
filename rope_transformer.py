...
class RoPEEmbedding(nn.Module):
    def __init__(self, d_model, max_seq_len):
        ...
        freqs = 1.0 / (10000 ** (torch.arange(0, d_model, 2).float() / d_model)).to(device)
        self.register_buffer("freqs", freqs)
        
        # Precompute position encodings
        positions = torch.arange(max_seq_len).float().to(device)
    ...
    def apply_rotary_pos_emb(self, q, k):
        ...
        :param q: query tensor [batch_size, num_heads, seq_len, d_k]
        :param k: key tensor [batch_size, num_heads, seq_len, d_k]
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model, num_heads, num_positions, dropout=0.1):
        ...
        self.rope = RoPEEmbedding(self.d_k, self.num_positions)
    ...
    def forward(self, query, key, value, mask=None):
        ...
        # Linear projections
        Q = self.W_q(query).view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1, 2) # [B, H, T, d_k]
        K = self.W_k(key).view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1, 2)
        V = self.W_v(value).view(batch_size, seq_len, self.num_heads, self.d_k).transpose(1, 2)
        
        # Apply RoPE by chaning dimensions
...
def train_rope_classifier_glue(train, val, num_epochs,save_path,  num_positions = 512, num_classes =2 , loss = "CE", use_token_type_embeddings =True):
    """
...
def decode_rope_glue(state_dict, dataloader,num_positions = 512, num_classes =2 , loss = "CE",  use_token_type_embeddings =True):
    model_config = {
        'vocab_size': tokenizer.vocab_size,  # BERT vocab size, adjust based on your tokenizer
        'num_positions': num_positions,
        'd_model': 256,
...
def attention_plots_rope(state_dict, valdata, save_path, num_positions = 512, num_classes =2 , use_token_type_embeddings =True):
    ...
    with torch.no_grad():
            ...
            for j in range(len(attention_maps)):
                for i, attn_map in enumerate(attention_maps[j]):
                    ...
                    plt.tight_layout()
                    plt.savefig(f"{save_path}/rope_sample{i}_attn_{j}.png")
                    plt.close(fig)
