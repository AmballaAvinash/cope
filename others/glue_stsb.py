...
train_encodings = tokenizer(train_sentences1, train_sentences2, truncation=True, padding="max_length", max_length=128)
val_encodings = tokenizer(val_sentences1, val_sentences2, truncation=True, padding="max_length", max_length=128)
...
if mode == "train": 
    for variant in ["real", "magnitude", "phase", "hybrid", "hybrid_norm"]: #,
            ...
            for i in range(N_exp):
                ...
                model, train_loss, val_loss = train_complex_classifier_glue(train = train_loader, val = val_loader, num_classes =1, loss = "MSE", num_epochs = num_epochs,variant = variant, save_path = "models_stsb/complex_{}_state_dict.pth".format(variant), use_token_type_embeddings =True)
            ...
            df.to_csv(f"loss_stsb/train_loss_complex_glue_{variant}.csv", index=False)

            df = pd.DataFrame(val_loss_complex, columns=[f"Epoch_{i+1}" for i in range(val_loss_complex.shape[1])])
            df.to_csv(f"loss_stsb/val_loss_complex_glue_{variant}.csv", index=False)
    ...
    #     model, train_loss, val_loss = train_rope_classifier_glue(train = train_loader, val = val_loader, num_classes =1, loss = "MSE",num_epochs = num_epochs, save_path = "models_stsb/rope_state_dict.pth",use_token_type_embeddings =True)
    ...
    # df.to_csv("loss_stsb/train_loss_rope_glue.csv", index=False)

    # df = pd.DataFrame(val_loss_rope, columns=[f"Epoch_{i+1}" for i in range(val_loss_rope.shape[1])])
    # df.to_csv("loss_stsb/val_loss_rope_glue.csv", index=False)
    ...
    #     model, train_loss, val_loss = train_learned_classifier_glue(train = train_loader, val = val_loader,num_classes =1, loss = "MSE", save_path="models_stsb/learned_state_dict.pth", num_epochs = num_epochs, use_token_type_embeddings =True)
    ...
    # df.to_csv(f"loss_stsb/train_loss_learned_glue.csv", index=False)
    # df = pd.DataFrame(val_loss_learned, columns=[f"Epoch_{i+1}" for i in range(val_loss_learned.shape[1])])
    # df.to_csv(f"loss_stsb/val_loss_learned_glue.csv", index=False)
    ...
    #     model, train_loss, val_loss = train_sine_classifier_glue(train = train_loader, val = val_loader,num_classes =1, loss = "MSE", num_epochs = num_epochs, save_path = "models_stsb/sine_state_dict.pth", use_token_type_embeddings =True)
    ...
    # df.to_csv(f"loss_stsb/train_loss_sine_glue.csv", index=False)

    # df = pd.DataFrame(val_loss_sine, columns=[f"Epoch_{i+1}" for i in range(val_loss_sine.shape[1])])
    # df.to_csv(f"loss_stsb/val_loss_sine_glue.csv", index=False)
    ...
    for variant in ["real", "magnitude", "phase", "hybrid", "hybrid_norm"]: #,
        ...
        state_dict = torch.load("models_stsb/complex_{}_state_dict.pth".format(variant))
        _ = decode_complex_glue(state_dict, val_loader, variant,num_classes =1, loss = "MSE")

    print("RoPE")
    state_dict = torch.load("models_stsb/rope_state_dict.pth")
    _ = decode_rope_glue(state_dict, val_loader,num_classes =1, loss = "MSE")

    print("learned")
    state_dict = torch.load("models_stsb/learned_state_dict.pth")
    _ = decode_learned_glue(state_dict, val_loader,num_classes =1, loss = "MSE")

    print("sine")
    state_dict = torch.load("models_stsb/sine_state_dict.pth")
    _ = decode_sine_glue(state_dict, val_loader,num_classes =1, loss = "MSE")
    ...
    for variant in ["real", "magnitude", "phase", "hybrid", "hybrid_norm"]: #,
        state_dict = torch.load("models_stsb/complex_{}_state_dict.pth".format(variant))
        attention_plots_complex(state_dict, val_dataset[0:1], variant, save_path = "attention_plots_stsb")

    state_dict = torch.load("models_stsb/rope_state_dict.pth")
    attention_plots_rope(state_dict, val_dataset[0:1], save_path = "attention_plots_stsb")

    state_dict = torch.load("models_stsb/learned_state_dict.pth")
    attention_plots_learned(state_dict, val_dataset[0:1], save_path = "attention_plots_stsb")

    state_dict = torch.load("models_stsb/sine_state_dict.pth")
    attention_plots_sine(state_dict, val_dataset[0:1],  save_path = "attention_plots_stsb")
